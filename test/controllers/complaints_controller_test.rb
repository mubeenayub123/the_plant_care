...
class ComplaintsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create complaint" do
    assert_difference("Complaint.count") do
      post complaints_url, params: { complaint: { description: @complaint.description, irrigation_system_id: @complaint.irrigation_system_id, status: @complaint.status, title: @complaint.title, user_id: @complaint.user_id } }
    end
  ...
  test "should update complaint" do
    patch complaint_url(@complaint), params: { complaint: { description: @complaint.description, irrigation_system_id: @complaint.irrigation_system_id, status: @complaint.status, title: @complaint.title, user_id: @complaint.user_id } }
